...
def CreateRectangleFrames(rectFrame, centerFrame, i=0):
    ...
    rectFrame = ctk.CTkFrame(
        master=centerFrame,
        height=(currentListToSort[0].listToSort[i] * 3.5),
        width=23,
        border_color="#52796F",
        border_width=1,
        fg_color="#84A98C",
    )
    rectLabel = ctk.CTkLabel(
        master=rectFrame,
        text=str(currentListToSort[0].listToSort[i]),
        text_color="white",
        font=("Great Vibes", 15),
    )
    ...
    rectangleWithValue = RectangleWithValue(
        rectFrame, currentListToSort[0].listToSort[i], rectLabel
    )
    ...
    if i + 1 < len(currentListToSort[0].listToSort):
        rectFrame.after(
            20,
            lambda frame=rectFrame, cFrame=centerFrame, index=(
                i + 1
            ): CreateRectangleFrames(frame, cFrame, index),
        )
