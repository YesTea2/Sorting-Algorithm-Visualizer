...
class SortList:
    ...
    def SelectSortList(self, number=0):
        ...
        if number >= len(listForSorting) - 1:
            ...
            PerformRainbow("SELECTION SORT FINISHED!")
        ...
        UpdateRectangleFrames(highlight_values + highlight_indices)

        # Recursive loop, runs after 1/2 of a second, recalling this method
        currentlyDisplayedMainFrame[0].after(150, lambda numToUse=(number+1):self.SelectSortList(numToUse))
    ...
    def UpdateBubbleSortVisual(self, listForSorting, currentIndex):
        ...
        if swappedIndices:
            ...
            UpdateRectangleFrames(values + swappedIndices)
...
def PerformRainbow(textToDisplay, index = 0):
    if index == 13:
        for rect in currentRectanglesOnScreen:
                rect.frame.configure(border_color="#52796F", fg_color="#84A98C")
        sentMessageLabel[0].configure(text=textToDisplay, text_color="white")
        currentlyDisplayedMainFrame[0].after(2200, ClearMessageInput)
        ...
        if index == 0 or index == 6:
            ...
            index += 1
            currentlyDisplayedMainFrame[0].after(25, lambda i=index, textMessage=textToDisplay:PerformRainbow(textMessage,i))
...
def CreateRectangleFrames(i=0):
    ...
    rectFrame = customtkinter.CTkFrame(master=currentCenterFrame[0], height=(currentListToSort[0].listToSort[i] *3.5), width=23, border_color="#52796F", border_width=1, fg_color="#84A98C")
...
def SearchForValue():
    ...
    if isCurrentlySortingData:
        DisplayMessage("WAIT FOR CURRENT ACTION TO FINISH")
        return
    # Data is not sorted, alert user, return
    if not isDataSorted:
        DisplayMessage("SORT DATA BEFORE SEARCH")
        return
    ...
    DisplayMessage("RUNNING BINARY SEARCH")
    ...
    currentCenterFrame[0].after(100, lambda x=xValue, minV=0, maxV=(len(currentListToSort[0].listToSort)-1): BinarySearch(x, minV, maxV))
...
def BinarySearch(xValue, minValue, maxValue):
    ...
    if maxValue < minValue:
        # Value not found
        DisplayMessage("Value not found")
        return
    ...
    if currentListToSort[0].listToSort[midValue] == xValue:
        # Value found!
        DisplayMessage(f"VALUE {xValue}, FOUND!")
        # Refresh the rectangles, highlight the value
        UpdateRectangleFrames([xValue])
        currentCenterFrame[0].after(2000, lambda value=xValue:PerformRainbow(f"VALUE {value} FOUND!"))
    ...
    # If the value in the middle is greater than the users inputted value
        ...
        UpdateRectangleFrames([minValue, midValue, maxValue])
        DisplayMessage(f"CHECKING MIN={minValue}, MID={midValue} MAX={maxValue}")
        ...
        currentCenterFrame[0].after(2000, lambda x=xValue, minV=minValue, maxV=(midValue - 1): BinarySearch(x, minV, maxV))
    # Else if the middle value is less than the inputted value
        ...
        currentCenterFrame[0].after(2000, lambda x=xValue, minV=(midValue + 1), maxV=maxValue: BinarySearch(x, minV, maxV))
...
def RunSelectionSort():
    ...
    if isDataSorted:
        DisplayMessage("RESET DATA BEFORE SORTING")
        return
    ...
    DisplayMessage("RUNNING SELECTION SORT")
...
def RunBubbleSort():
    ...
    DisplayMessage("RUNNING BUBBLE SORT")
...
def RunMergeSort():
    ...
    # Ready to run bubble sort
    DisplayMessage("RUNNING MERGE SORT")
    # Run a bubble sort on the current list inside the SortList object
...
def CreateErrorFrame():
    ...
    messageInput = customtkinter.CTkLabel(master=currentlyDisplayedMainFrame[0], text="", text_color="white", font=("Great Vibes", 30))
    # Packing it to the screen
    messageInput.pack(side="top", pady=(0,25), expand=False)
    # Adding it to its list
    sentMessageLabel.append(messageInput)
...
def DisplayMessage(textToDisplay:str):
    ...
    sentMessageLabel[0].configure(text=textToDisplay)
    # Updating the label
...
def ClearMessageInput():
    ...
    sentMessageLabel[0].configure(text="")
    # Updating the label
    sentMessageLabel[0].update_idletasks()
